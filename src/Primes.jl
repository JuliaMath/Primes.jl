...
if VERSION >= v"0.5.0-dev+4340"
    ...
    function checked_neg(x::Integer)
        y = -x
        if y < 0 && x < 0 throw(OverflowError()) end
        return y
    end
end
...
function _primesmask(limit::Int)
    if limit < 7 throw(ArgumentError("Limit must be at least 7, got $limit.")) end
    n = wheel_index(limit)
    m = wheel_prime(n)
    sieve = ones(Bool, n)
...
function _primesmask(lo::Int, hi::Int)
    if !(7 ≤ lo ≤ hi) throw(ArgumentError("The condition 7 ≤ lo ≤ hi must be met.")) end
    if lo == 7 return _primesmask(hi) end
    wlo, whi = wheel_index(lo - 1), wheel_index(hi)
    m = wheel_prime(whi)
    sieve = ones(Bool, whi - wlo)
    if hi < 49 return sieve end
...
function primesmask(lo::Int, hi::Int)
    if !(0 < lo ≤ hi) throw(ArgumentError("The condition 0 < lo ≤ hi must be met.")) end
    sieve = falses(hi - lo + 1)
    if lo ≤ 2 ≤ hi sieve[3 - lo] = true end
    if lo ≤ 3 ≤ hi sieve[4 - lo] = true end
    if lo ≤ 5 ≤ hi sieve[6 - lo] = true end
    if hi < 7 return sieve end
...
function primes(lo::Int, hi::Int)
    if !(lo ≤ hi) throw(ArgumentError("The condition lo ≤ hi must be met.")) end
    list = Array{Int, 1}()
    if lo ≤ 2 ≤ hi push!(list, 2) end
    if lo ≤ 3 ≤ hi push!(list, 3) end
    if lo ≤ 5 ≤ hi push!(list, 5) end
    if hi < 7 return list end
    lo = max(2, lo)
...
function isprime(n::Integer)
    ...
    sP = [2, 3, 5, 7, 11, 13, 17, 19, 23]
    for m in sP if n % m == 0 return n == m end end
    if n < 841 return n > 1 end
...
```
"""
isprime(x::BigInt, reps::Integer = 25) = ccall((:__gmpz_probab_prime_p,:libgmp), Cint, (Ptr{BigInt}, Cint), &x, reps) > 0
...
const bases = UInt16[15591,  2018,   166,  7429,  8064, 16045, 10503,  4399,  1949,  1295,  2776, 3620,   560,  3128, 5212,
                      ...
                      4852,  5012,  3185,  1333,  6227,  5298,  1074,  2391,  5113,  7061,   803, 1269,  3875,   422,  751,
                       580,  4729, 10239,   746,  2951,   556,  2206,  3778,   481,  1522,  3476,  481,  2487,  3266, 5633,
                       488,  3373,  6441,  3344,    17, 15105,  1490,  4154,  2036,  1882,  1813,  467,  3307, 14042, 6371,
                       658,  1005,   903,   737,  1887,  7447,  1888,  2848,  1784,  7559,  3400,  951, 13969,  4304,  177,
                        41, 19875,  3110, 13221,  8726,   571,  7043,  6943,  1199,   352,  6435,  165,  1169,  3315,  978,
                       233,  3003,  2562,  2994, 10587, 10030,  2377,  1902,  5354,  4447,  1555,  263, 27027,  2283,  305,
                       669,  1912,   601,  6186,   429,  1930, 14873,  1784,  1661,   524,  3577,  236,  2360,  6146, 2850,
                     55637,  1753,  4178,  8466,   222,  2579,  2743,  2031,  2226,  2276,   374, 2132,   813, 23788, 1610,
                      4422,  5159,  1725,  3597,  3366, 14336,   579,   165,  1375, 10018, 12616, 9816,  1371,   536, 1867,
                     10864,   857,  2206,  5788,   434,  8085, 17618,   727,  3639,  1595,  4944, 2129,  2029,  8195, 8344,
                      6232,  9183,  8126,  1870,  3296,  7455,  8947, 25017,   541, 19115,   368,  566,  5674,   411,  522,
                      1027,  8215,  2050,  6544, 10049,   614,   774,  2333,  3007, 35201,  4706, 1152,  1785,  1028, 1540,
                      3743,   493,  4474,  2521, 26845,  8354,   864, 18915,  5465,  2447,    42, 4511,  1660,   166, 1249,
                      6259,  2553,   304,   272,  7286,    73,  6554,   899,  2816,  5197, 13330, 7054,  2818,  3199,  811,
...
function factor!{T<:Integer,K<:Integer}(n::T, h::Associative{K, Int})
    ...
    if n < 0
        ...
        if isa(n, BitSigned) && n == typemin(T)
            h[2] = 8 * sizeof(T) - 1
            return h
        else return factor!(checked_neg(n), h) end
    elseif n == 0 || isprime(n) h[n] = 1; return h
    elseif n == 1 return h end
    ...
    for p in PRIMES
        if n % p == 0
            ...
            if n == 1 return h end
            if isprime(n) h[n] = 1; return h end
        end
    end
...
function pollardfactors!{T<:Integer,K<:Integer}(n::T, h::Associative{K, Int})
    while true
        ...
        while c == n - 2 c = rand(1:(n - 1)) end
        while G == 1
            x = y
            for i in 1:r y = (powermod(y, 2, n) + c) % n end
            local k::K = 0
            G = 1
            while k < r && G == 1
...
function lucaslehmer(P::Integer, s::Integer = BigInt(4))
    if P < 3 return throw(ArgumentError("The condition P ≥ 3 must be met.")) end
    M = BigInt(2)^P - 1
    for i in 1:(P - 2) s = (s^2 - 2) % M end
    return s == 0
end
...
function riesel(k::Integer, n::Integer)
    if !(0 < k < BigInt(2)^n) return throw(ArgumentError("The condition 0 < k < 2^n must be met.")) end
    if k == 1 && n & 1 == 1 return n % 4 == 3 ? lucaslehmer(n, BigInt(3)) : lucaslehmer(n)
    elseif k == 3 && (n % 4) % 3 == 0 return lucaslehmer(n, BigInt(5778))
    elseif (k % 6) % 4 == 1 && (k * 2^n - 1) % 3 > 0 pass = 1 end  # TBA.
